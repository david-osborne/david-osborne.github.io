...
export class cStars {
  ...
  generateStars(theGrid: any[], gridDim: number) {
    ...
    theGrid.forEach(grid => {
      for (let i = 0; i < starCount; i++) {
        ...
        if (twinkleChance > 60)
          twinkleOn = 1;
        let starX = csFunctions.randomInt(0, gridDim),
          starY = csFunctions.randomInt(0, gridDim),
          starR = csFunctions.randomInt(1, 2),
          starOpacity = csFunctions.randomInt(1, 100), //math random generates between 0 and 1
  ...
  drawStars(ctx: any, theGrid: any[], gridDim: number, gridIndex: number, gridX: number, gridY: number) {
